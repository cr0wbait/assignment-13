...
function showStats(initNum, [numRA]){
  ...
  for (i=0;i<numRA.length;i++){
    statOb.setSize = numRA.length;
    if (numRA[i] === initNum){
      statOb.count_inputEqualTo = statOb.count_inputEqualTo + 1;
    }
    if (numRA[i] < initNum){
      statOb.count_inputGreaterThan = statOb.count_inputGreaterThan + 1;
    }
    if (numRA[i] > initNum){
      statOb.count_inputLessThan = statOb.count_inputLessThan + 1;
    }
  }
  return statOb;
}
